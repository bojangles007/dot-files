...
* Gnus setup
  ...
  Add a key binding to start an offlineimap run from Gnus.

  #+BEGIN_SRC emacs-lisp
  (define-key gnus-group-mode-map "o" 'offlineimap)
  (define-key gnus-group-mode-map "O" 'offlineimap-kill)
  #+END_SRC
...
* Composing
  ...
  This requires the [[file:README.org::*Start%20the%20Emacs%20Daemon][Emacs server to be running]].
