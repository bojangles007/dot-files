...
*** Setup Packages
    ...
    First we initialize the already-installed optional packages, whatever they
    are. Normally this would be called just before the =after-init-hook=. We
    do it here to ensure any such packages are available for the rest of this
    file.
    ...
    Disable the regular =(package-initalize)= run as it will be unnecessary now.
