...
**** TODO Remove this section when CLJ Refactor has been fixed
   ...
   The Clojure Refactor package is also ace. It seems to have temporarily lost
   its dependency spec, so let's first install those:

   #+BEGIN_SRC emacs-lisp
     (use-package paredit :ensure t)
     (use-package edn :ensure t)
     (use-package inflections :ensure t)
     (use-package hydra :ensure t)
   #+END_SRC

   Then Clojure Refactor itself:
...
** Org
   ...
   I also like to use the [[https://github.com/howardabrams/dot-files/blob/master/elisp/ox-confluence.el][Confluence exporter]] for pasting into JIRA comments:
